...
CEILINGS = {
    "features/yes-no.storyarc": 18528,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "features/press-any-key.storyarc": 19176,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "features/shiftable.storyarc": 18664,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "features/enhance-redefine.storyarc": 19032,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "features/consult-about.storyarc": 19164,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "features/session-verbs.storyarc": 18804,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "features/vary.storyarc": 20016,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "features/foresight.storyarc": 19948,  # 2026-07-29 repriced: thing-dual words (a command word that is also thing vocabulary answers can-t-see out of scope; the __tduals__ table and its walk)
    "features/beyond.storyarc": 20760,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "features/alter.storyarc": 19368,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "features/catalogs.storyarc": 19320,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "features/matrix.storyarc": 19668,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "features/direction-grammar.storyarc": 18712,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "features/scenery-contents.storyarc": 19220,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "granules/nautical.storyarc": 19176,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    ...
    "beispiel-deutsch.storyarc": 25004,  # 2026-07-29 repriced: thing-dual words (a command word that is also thing vocabulary answers can-t-see out of scope; the __tduals__ table and its walk)
    "brass-lantern.storyarc": 19920,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "cloak-of-darkness.storyarc": 20904,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "ejemplo-espanol.storyarc": 23712,  # 2026-07-29 repriced: phrase_named tests the grain flag (out-of-scope scenery words answer can-t-see, not the ask)
    "features/computed-properties.storyarc": 18412,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "features/containers.storyarc": 18756,  # 2026-07-29 repriced: thing-dual words (a command word that is also thing vocabulary answers can-t-see out of scope; the __tduals__ table and its walk)
    "features/daemons-and-timers.storyarc": 20000,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "features/doors-and-locks.storyarc": 18324,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "features/appearance.storyarc": 19136,  # 2026-07-29 repriced: thing-dual words (a command word that is also thing vocabulary answers can-t-see out of scope; the __tduals__ table and its walk)
    "features/components.storyarc": 18556,  # 2026-07-29 repriced: thing-dual words (a command word that is also thing vocabulary answers can-t-see out of scope; the __tduals__ table and its walk)
    "features/perform.storyarc": 18584,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "features/grains.storyarc": 18476,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "features/handlers.storyarc": 19556,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "features/grammar.storyarc": 18704,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "features/introproperty.storyarc": 19632,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "features/kinds-and-inheritance.storyarc": 18388,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "features/on-other.storyarc": 18268,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "features/zcolor.storyarc": 18648,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "features/scoring.storyarc": 20664,  # 2026-07-29 repriced: thing-dual words (a command word that is also thing vocabulary answers can-t-see out of scope; the __tduals__ table and its walk)
    "features/spans.storyarc": 18492,  # 2026-07-29 repriced: thing-dual words (a command word that is also thing vocabulary answers can-t-see out of scope; the __tduals__ table and its walk)
    "features/vehicles.storyarc": 18972,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "granules/ambience.storyarc": 20424,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "granules/conversations.storyarc": 20092,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "granules/extended-verbs.storyarc": 21340,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "granules/infocom-interrogation.storyarc": 20556,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "granules/quotes.storyarc": 18652,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "granules/take-all.storyarc": 20308,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "granules/plurals.storyarc": 19260,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "granules/statusline.storyarc": 18380,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
    "granules/verbose-exits.storyarc": 18656,  # 2026-07-29 repriced DOWN: the exact-objective abbreviation optimizer and the DP encoder parse (the win locked in)
}
