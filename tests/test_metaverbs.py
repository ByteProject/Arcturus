...
def test_unknown_verb_and_quit_on_frotz(tmp_path):
    ...
    assert "Those words don't add up to anything." in out  # unknown verb gets a reply
    assert "Are you sure you want to quit?" in out  # quit asks for confirmation
    assert "We'll leave it there." in out  # confirmed quit says the farewell
...
def test_quit_can_be_cancelled_on_frotz(tmp_path):
    ...
    assert "We'll leave it there." not in out  # declined - no farewell
