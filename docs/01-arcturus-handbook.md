...
| `article` | text | The definite article, verbatim, when derivation cannot reach it: `article "las"` (las tijeras), `article "el"` (el agua), `article "a swarm of"`. Store it lowercase; the compiler synthesizes a capitalized twin, so the object opens a sentence correctly ("A swarm of bees is not on the menu."). |
| `indefinite` | text | The indefinite article, verbatim: `indefinite "unas"`, or an English mass noun with `indefinite "some"` ("You can see some water here."). Lowercase, with the same compiler-made capitalized twin for sentence starts. |
