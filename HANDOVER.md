...
| B8 | Port Hibernated 2 (the maturity milestone) | done: plays to its ending, smaller than the Inform build |
| B9 | Port Ghosts of Blackwood Manor | DROPPED 2026-07-04 (Stefan's call; B8 made its proof redundant; number stays reserved) |
...
(`python3 -m pytest`). Working tree clean at commit 37908cb. The living log
...
- B8: the flagship ports completely and plays to its ending. The port
  ...
  past 128K; direction or-lists). The B8 source is Stefan's commercial
...
- Conformance: CZECH 406/406 byte-matched, Praxix all-pass, TerpEtude text
  ...
  the B8 game end to end: all in the suite. `actaea/conformance/` holds third-party
...
harness: `-p -w 200 -h 8000`, blank line = keypress). The B8 game's full
...
run the suite, play the B8 game on all three front-ends, and bring back a
