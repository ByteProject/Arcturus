; dzx0_6502.asm - ZX0 (v2 stream) decompressor for the 6502/6510
;
...
;
; entry:  X = source lo, A = source hi, .lz_dst/.lz_dst+1 = destination
; exit:   at the stream's end marker; trashes A/X/Y and the zp cells.
;
; Upstream's own warning stands: this decodes the ORIGINAL zx0 v2 stream,
; not bitfire's speed-modified variant.
...
.depacker_start
		...
		;the DESTINATION is preloaded by the caller in zp .lz_dst
		;(the one adaptation against upstream, which hardwired $a000)
		sta .lz_src1
		sta .lz_src2
		sta .lz_src3
