...
class StringInterp:
    ...
    # leading article helper (a, an, the, A, An, The); see docs/01 chapter 15.
...
class IsTest(Expr):
    ...
    test or an equality is decided in semantic analysis (docs/01 chapter 9)."""
...
@dataclass
class Summon:
    # The three summon forms (docs/01 chapter 22). `target` is a feature id, a granule
...
# Property declaration forms (docs/01 chapter 5).
...
class Handler:
    ...
    # one entry. `other` is the catch-all (docs/01 chapter 11).
    events: list[str]
    after: bool = False
...
class SubjectDecl:
    ...
    character that raises it (docs/01 chapter 17): the match words and the
...
class ChainDecl:
    ...
    # next command once the current one succeeds (docs/01 chapter 14). All chain
...
class AllDecl:
    ...
    # translation forks the granule and redeclares them (docs/01 chapter 21).
    words: list[str]
    line: int = 0
...
class GlobalDecl:
    ...
    # says what you are holding (docs/01 chapter 2).
    name: str
    value: Expr
    role: str = "global"
    line: int = 0
...
@dataclass
class AmbienceBlock:
    # An `ambience` block on a room or thing (summon.ambience, docs/01 chapter 22): a
...
class GrainsAttach:
    ...
    <object>.grains (docs/01 chapter 18)."""
