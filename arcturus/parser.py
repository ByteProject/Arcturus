...
runtime constructs (grains attach, scheduling) from docs/01 chapters 16 and 18.
...
class Parser:
    ...
    def _kind_name(self, what: str) -> str:
        ...
        `thing` and `room`, which are keywords (docs/01 chapter 3)."""
        if self.cur.kind == T.NAME:
    ...
    def parse_toplevel(self) -> ast.Decl:
        ...
        if t.value == "player" and t.kind in (T.NAME, T.KW):
            ...
            if nxt.kind == T.OP and nxt.value == ".":
                ...
                # block: augment the seeded player object (docs/01 chapter 4).
                line = t.line
                self.advance()
                self.advance()
    ...
    def parse_subject(self) -> ast.SubjectDecl:
        ...
        # (docs/01 chapter 17). A subject is declared once at file level and
    ...
    def parse_chain(self) -> ast.ChainDecl:
        ...
        # (docs/01 chapter 14). Dispatched as a leading name, like direction and
    ...
    def parse_statement(self) -> ast.Stmt:
        ...
        if t.kind == T.NAME:
            ...
            # statement (docs/01 chapter 15). Dispatched here because zcolor is
            # not a reserved word.
            if t.value == "zcolor":
