...
them (docs/01 appendix C). Cosmos is written in Arcturus and will be compiled
...
spec-derived (docs/01 appendix C). Semantic analysis takes the environment as
...
# Standard kinds and their parent chain (docs/01 chapter 3, appendix A).
_STD_KINDS = [
    StdKind("thing", None),
    StdKind("room", None),
...
_STD_BOOL_PROPS = [
    ...
    # all: that is a grain's job (docs/01 chapter 18).
    "beyond",
...
# Standard action names a verb grammar line can produce (docs/01 appendix C).
_STD_ACTIONS = {
...
# Engine-fired events, plus the catch-all (docs/01 chapters 11 and 13).
_CORE_EVENTS = {"start", "enter", "each_turn", "other"}

# Builtin references usable in any handler or block (docs/01 chapter 2).
_BUILTINS = {
    "player": T_OBJECT,
    "here": T_OBJECT,
    "self": T_OBJECT,
    ...
    # The pronoun referents, one per canonical role (docs/01 chapter 14): what
    ...
    # swaps the roles. Library-internal (docs/01 chapter 14).
    "two_reverse": T_NUMBER,
    # Set by the positional grammar matcher (docs/01 chapter 14) to the matched
    ...
    "refused": T_NUMBER,
    # Command chaining (docs/01 chapter 14), library-internal: the text-buffer
    ...
    # The disambiguation ask (docs/01 chapter 14), library-internal: the tied
    ...
    "all_go": T_NUMBER,
    # The plurals granule (docs/01 chapter 22): plural_go carries the matched group word
