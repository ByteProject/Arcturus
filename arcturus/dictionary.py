...
# and answers with the grain whose owner is in scope (docs/01 chapter 18).
_SCENERY_FLAG = 0x10
...
# words after it run as the next command once this one succeeds (docs/01 chapter 14).
...
# A verb with a positional grammar table (docs/01 chapter 14): the verb bit
...
def collect_vocab(world: wm.World) -> set:
    ...
    # A free-standing LIST's entries are vocabulary by declaration: a
    # table of forms a language pack rewrites INTO needs no object to
    # own them (docs/01, chapter 5's list type at top level).
    for lst in world.lists.values():
        words.update(lst.words)
