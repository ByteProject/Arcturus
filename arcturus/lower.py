...
INTRINSICS = frozenset({
    ...
    "arc_image_dark", "any_dark", "any_shiftable", "any_notify",
...
# Builtin references that hold an object number at run time (docs/01 chapter 2).
...
def _intrinsic(rt, ctx, call: ast.Call, dest):
    ...
    if name == "read_line":
        ...
        _place(rt, Const(0), dest)
    elif name == "any_notify":
        # any_notify(): 1 when the game writes the notify global anywhere,
        # so the loop's score-watch folds away otherwise (the coupled rule).
        _place(rt, Const(1 if ctx.world.uses_notify else 0), dest)
        ...
        if len(args) != 1 or not isinstance(args[0], ast.StringLit):
            raise LowerError(
                'dict_entry takes one quoted word: dict_entry("se")',
                call.line)
        word = _plain_string(args[0])
        if word is None or len(word.strip().split()) != 1:
            raise LowerError(
                "dict_entry takes exactly one word", call.line)
...
def _distribute_is_list(ctx, expr):
    """The is-list (docs/01 chapter 9): `way is aft or north` distributes to
...
def _any_unruled(ctx) -> int:
    ...
    for h in world.free_handlers:
        ...
        # A free handler rules its action only when it answers EVERY arrival:
        # one with an object filter (`on set dial`) or a when guard answers
        # some commands and lets the rest fall through, so the tail must
        # stay for them (SET LAMP was a silent turn, found by the typed
        # example's own TRY script).
        if getattr(h, "pattern", None) or getattr(h, "when", None):
            continue
...
def _emit_prop_print_or_run(rt, ctx, dot, add_newline):
    ...
    "print or run" the computed-property read needs (docs/01 chapter 5).
...
def _static_value(ctx, expr):
    ...
    if isinstance(expr, ast.Call) and not expr.args and expr.name == "any_notify":
        return 1 if ctx.world.uses_notify else 0
...
def _schedule(rt, ctx, s: ast.Schedule):
    """Arm a timer (docs/01 chapter 16). `after N turns do B` sets B's slot to a
...
def _stop_schedule(rt, ctx, s: "ast.StopSchedule"):
    """Disarm the timer the full triple names (docs/01 chapter 16): the kind
