...
def actions_with_after(world: "World") -> list:
    ...
    the same chain once the real action completes unrefused (docs/01 chapter 13)."""
    found = set()
...
class Obj:
    ...
    grains: list[Grain] = field(default_factory=list)
    topics: list["ast.TopicDecl"] = field(default_factory=list)
    # The object's ambience blocks (summon.ambience, docs/01 chapter 22), kept as their
...
# How an `is` test resolved (docs/01 chapter 9).
...
class World:
    ...
    # Conversation subjects declared at file level (docs/01 chapter 17): id ->
    ...
    # The language layer's crossword folds (`fold "ä" "ae"`, docs/01 chapter
    # 21): (source, replacement) pairs the dictionary build applies to every
    # word to register the folded sibling spelling. Empty for a pack that
    # declares none (English, Spanish), and the machinery costs nothing then.
