...
def _convert_c64(rows, salient=None):
    # Multicolor bitmap by PIXEL POLIZEI'S RECIPE (m_c64mc.pde, WTFPL,
    # Markku Reunanen), DIRECT from the master, Stefan's correction: no
    # intermediate. Every pixel maps plainly to Colodore (restored by his
    # ruling: the vivid palette read genuinely well, and Pepto holds no
    # teal and no hot pink; real-hardware truth is the probe's job), the
    # background is the most frequent color among CLASHING blocks only,
    # and a clashing block keeps its three most frequent colors plus the
    # background, remapped locally. Frequency and locality, no global
    # optimizer. This conversion is THE BASE OF THE 8-BIT FAMILY: the A8
    # (and later siblings) derive from it, as the 80s ports derived from
    # the C64 version.
    idx = _collapse_pairs([[_nearest(c, _COLODORE) for c in row]
                           for row in rows])
    h, w = len(idx), len(idx[0])
    # The master reference at halved width (pair averages; legitimate
    # here, it is only the dither's yardstick, never a palette input).
    refs = [[tuple((a + b) // 2 for a, b in zip(row[x], row[x + 1]))
             for x in range(0, len(row), 2)] for row in rows]
    seam = _seam_mask(refs)
    forced = set()
    if salient:
        _force_disc(idx, salient, 1)  # the disc goes white, the R3 ruling
        cells = {}
        for x, y in salient:
            cells[(x // 2, y)] = cells.get((x // 2, y), 0) + 1
        forced = {k for k, n in cells.items() if n == 2}
    ...
    for cy in range(cells_y):
        for cx in range(cells_x):
            ...
            free = [c for c in sorted(hist, key=hist.get, reverse=True)
                    if c != bg][:3]
            allowed = [bg] + free
            argb = [_COLODORE[c] for c in allowed]
            for yy in range(8):
                for xx in range(4):
                    px, py = cx * 4 + xx, cy * 8 + yy
                    if (px, py) in forced:
                        # The disc stays solid; spice never speckles it.
                        pixels[py][px] = allowed.index(1) if 1 in allowed \
                            else _nearest(_COLODORE[1], argb)
                        continue
                    pixels[py][px] = _spice(refs[py][px], argb, px, py,
                                            seam[py][px])
            free += [bg] * (3 - len(free))
...
def _convert_cpc(rows, salient=None):
    # Master in, C64 taste and dither first, CPC out; the master rides
    # along as the de-grey yardstick (pair averages, reference only).
    refs = [[tuple((a + b) // 2 for a, b in zip(row[x], row[x + 1]))
             for x in range(0, len(row), 2)] for row in rows]
    return _cpc_from_c64(_convert_c64(rows, salient), refs,
...
def _convert_a8(rows, salient=None):
    ...
    # the master rides along as the de-grey yardstick.
    refs = [[tuple((a + b) // 2 for a, b in zip(row[x], row[x + 1]))
             for x in range(0, len(row), 2)] for row in rows]
    return _a8_from_c64(_convert_c64(rows, salient), refs,
                        _forced_cells(salient))
...
def _convert_p4(rows, salient=None):
    # Hires TED, THE PHOTOSHOP MANNER (Stefan's reboot ruling, 2026-07-23,
    # after the region-model pilot failed his eye). The working recipe is
    # the one his own Ghosts of Blackwood Manor conversions prove: choose
    # ONE small adaptive palette for the whole image, expressed in the
    # machine's own colours; error-diffuse the entire master against it
    # (dither is the universal medium, not a rationed effect; no Bayer
    # crosses); THEN, and only then, resolve the hardware constraint per
    # cell, each 8x8 choosing the legal pair that best preserves the
    # dithered image (the ZX3 solver's manner: frequency candidates plus
    # the pivot pair). Global palette consistency means neighbouring
    # cells cannot invent clashes, and the brightest cluster is protected
    # into the palette (_protect_extremes), which is what keeps the moon
    # without any hint. `salient` is accepted and ignored.
    h, w = len(rows), len(rows[0])
    ladder = _TED_LUMA

    # The machine's voice: the full TED colour set, black plus fifteen
    # hues on the eight-step luminance ladder.
    ted_rgb = [(0, 0, 0)]
    ted_hl = [(0, 0)]
    ...
    # palette entries, the leader against black, and the pivot pair from
    # the cell's own tonal structure. Score against the master's pixels;
    # assign each pixel by its DIFFUSED colour, so the dither survives
    # the constraint.
    cells_x, cells_y = w // 8, h // 8
    pixels = [[0] * w for _ in range(h)]
    screen = []
        ...
        for cx in range(cells_x):
            ...
            for yy in range(8):
                for xx in range(8):
                    ...
                    i = idx[py][px]
                    hist[i] = hist.get(i, 0) + 1
                    cell.append((rows[py][px], i, px, py))
            top = sorted(hist, key=hist.get, reverse=True)
            a = top[0]
            b = top[1] if len(top) > 1 else top[0]
            cands = {(a, b), (a, a)}
            # the pivot pair: split the cell at its median luminance,
            # average each side, snap each average into the palette
            lums = sorted((0.299 * c[0] + 0.587 * c[1] + 0.114 * c[2], c)
                          for c, _i, _x, _y in cell)
            piv = lums[len(lums) // 2][0]
            losd = [c for l, c in lums if l <= piv]
            hisd = [c for l, c in lums if l > piv] or losd
            avg = lambda g: tuple(sum(v[k] for v in g) / len(g)
                                  for k in range(3))
            cands.add((_nearest(avg(hisd), pal_rgb),
                       _nearest(avg(losd), pal_rgb)))
            best = None
            for ca, cb in cands:
                err = 0
                for c, _i, _x, _y in cell:
                    err += min(_dist(c, pal_rgb[ca]), _dist(c, pal_rgb[cb]))
                if best is None or err < best[0]:
                    best = (err, ca, cb)
            _e, ca, cb = best
            # ink is the brighter of the pair; a one-colour cell rides on
            # black paper
            la = 0.299 * pal_rgb[ca][0] + 0.587 * pal_rgb[ca][1] \
                + 0.114 * pal_rgb[ca][2]
            lb = 0.299 * pal_rgb[cb][0] + 0.587 * pal_rgb[cb][1] \
                + 0.114 * pal_rgb[cb][2]
            if ca == cb:
                ink_i, pap_i = ca, None
            elif la >= lb:
                ...
                ink_i, pap_i = cb, ca
            ink_rgb = pal_rgb[ink_i]
            pap_rgb = pal_rgb[pap_i] if pap_i is not None else black_rgb
            for c, i, px, py in cell:
                dc = pal_rgb[i]
                pixels[py][px] = 1 if _dist(dc, ink_rgb) \
                    <= _dist(dc, pap_rgb) else 0
            ih, il = pal_hl[ink_i]
            ph, pl = pal_hl[pap_i] if pap_i is not None else (0, 0)
            screen.append((ih << 4) | ph)
            color.append((il << 4) | pl)
    return {"w": w, "h": h, "pixels": pixels, "screen": screen,
            "color": color, "regs": [0]}
