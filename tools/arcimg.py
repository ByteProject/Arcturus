...
def _convert_zx3(rows, salient=None):
    ...
    # Pass 2: bright coherence. The master's cloud whites sit BETWEEN the
    # Spectrum's two whites, so independent cell picks oscillate: a grey
    # D7 cloud block and its darker cyan sky drop into an otherwise bright
    # sky (the blocky clash Stefan called out around rounded clouds). A
    # pixel artist commits the whole region to one bright level; this
    # smoothing does the same: a cell surrounded by a clear majority of
    # the other level flips to it unless its own error cost forbids. The
    # two whites are nearly interchangeable, so white-dominated cells
    # (cloud interiors) get a looser bound and the sky's level can
    # propagate through the whole cloud over the rounds.
    brights = [0 if pb[0][0] <= pb[1][0] else 1 for pb in choices]
    white_frac = []
...
def _convert_cpc(rows, salient=None):
    ...
    for row in rows:
        for c in row:
            hist[c] = hist.get(c, 0) + 1
    colors = list(hist)
    dmat = [[_dist(c, k) for k in cube] for c in colors]
    INF = 1 << 60
    ...
    # palette carries the salient colors, and the mix stays gentle. Flat
    # art stays undithered.
...
def _salient_pixels(rows, discs):
    ...
    for cx, cy, r in discs:
        ...
        if bright - dark < 200:      # uniform disc: all of it is the object
            ...
            # Occluded disc: the author's circle is the shape's truth, so
            # only clear occluders (trees in front, near the dark cluster)
            # are excluded. The low cut also keeps the artist's own
            # rim-dither pixels inside the disc, so the promoted shape
            # stays round instead of notched.
