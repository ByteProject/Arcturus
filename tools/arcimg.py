...
# THE TWO MASTER CLASSES (Stefan's ruling, 2026-09-13, Marco Innocenti's
# per-class masters the driving case): a portfolio may carry a second,
# broad-pixel master set in a masters-broad/ subdirectory beside the
# numbered masters. The BASE set serves the fine-pixel targets (dither
# survives); the broad set (no dither, committed tones) serves these
# targets, whose wide pixels or own halftone would eat the dither. A
# same-numbered picture in masters-broad/ overrides per broad target;
# an author with one set changes nothing. TRSM4 is broad although 640
# wide: its route halftones from luminance, and committed tones feed a
# clean halftone where pre-dithered speckle moires (same will hold for
# VDC when its converter lands, though VDC is ruled fine on paper).
_BROAD_TARGETS = frozenset({"C64", "P4", "CPC", "A8", "AP2", "TRSM4"})
...
def _convert_zx3_direct(rows, salient=None):
    """THE DIRECT ROUTE (--zx-colour; chartered 2026-09-13, committed-
    palette masters the driving case): art that is already Spectrum-
    shaped (flat palette regions over black, per-pixel texture the
    author placed himself) converts 1:1. Every master pixel lands where
    it stands; a lit pixel snaps to the Spectrum hue it NAMES, hue
    first and never luminance (an azure is blue however bright, where
    the luma-dominant metric handed it to magenta); each cell's ink is
    the color most of its lit pixels agree on, over black paper, the
    bright bit following the cell's own tier. No budget, no region
    coherence, no synthetic texture: the master is trusted whole."""
    rows = [row[_MS1_CROP_X:_MS1_CROP_X + 256] for row in rows]
    w, h = len(rows[0]), len(rows)
    ...
    def snap(c):
        m = max(c)
        if m < 32:
            return None                    # black is the paper
        scaled = tuple(255 * v // m for v in c)
        k = min(range(8, 15), key=lambda k: _dist(scaled, _ZX15[k]))
        return k if m >= 160 else k - 7    # the dim tier is the basic color
...
def _convert_ap2(rows, salient=None):
    """Master to Apple II DHGR, the signal class in full (Stefan's ruling,
    2026-08-17: the NTSC model first, the ii-pix lineage). The 280-column
    window (left-weighted, eight columns off the left, the MSX1 window
    principle) feeds a per-scanline dynamic program over all 560 dot
    positions: the NTSC decoder shows, at every dot, the colour named by
    the last four dots through the phase table, so the DP chooses each
    bit to minimise the distance between that colour and the master
    column beneath it, reaching colours BETWEEN the sixteen through
    sequences no aligned mapping can express. Plain RGB distance on the
    flat masters, the Polizei manner; the palette is _AP2_PAL, ii-pix's
    emulator-verified set."""
    rows = [r[8:288] for r in rows]
    h = len(rows)
    aux = [[0] * 40 for _ in range(h)]
    main = [[0] * 40 for _ in range(h)]
    INF = float("inf")
    for y in range(h):
        tgt = rows[y]
        # every palette entry's distance to every master column, once
        d = [[(c[0] - p[0]) ** 2 + (c[1] - p[1]) ** 2 + (c[2] - p[2]) ** 2
              for p in _AP2_PAL] for c in tgt]
        cost = [INF] * 8
        cost[0] = 0.0            # the dots left of the band are dark
        back = []
        for x in range(560):
            dc = d[x >> 1]
            idx = _AP2_IDX[x & 3]
            ...
            back.append(nback)
            cost = ncost
        st = min(range(8), key=lambda q: cost[q])
        bits = [0] * 560
        for x in range(559, -1, -1):
            bits[x] = st & 1
            st = back[x][st]
        for k in range(80):
            byte = 0
            for i in range(7):
                if bits[7 * k + i]:
                    byte |= 1 << i
            (aux if k % 2 == 0 else main)[y][k >> 1] = byte
    return {"w": 280, "h": h, "aux": aux, "main": main}
...
_CONVERTERS = {"AMI": _convert_ami, "AST": _convert_ast, "DOS": _convert_dos,
               ...
               "NXT": _convert_nxt, "M65": _convert_m65, "AP2": _convert_ap2}
...
# ---- AP2: DHGR, aux and main pages in display row order -----------------------

# The DHGR artifact palette in ii-pix's emulator-verified convention
# (OpenEmulator sRGB, frozen 2026-08-17, ending R1's "approximate" note):
# the colour at dot x is named by the last four dots, each contributing
# at bit (dot position mod 4). _AP2_IDX[x % 4][window] resolves a window
# (packed oldest-dot-highest) to the palette index; the aligned case
# (x % 4 == 3) is the classic nibble whose LSB is its leftmost dot.
_AP2_PAL = [
    (0, 0, 0), (20, 0, 246), (0, 150, 0), (0, 174, 243),
    (99, 103, 0), (130, 130, 130), (0, 235, 0), (25, 243, 136),
    (203, 0, 121), (230, 0, 244), (130, 130, 130), (160, 156, 244),
    (244, 78, 0), (244, 105, 235), (214, 218, 0), (244, 247, 244),
]


def _ap2_idx_table():
    tbl = [[0] * 16 for _ in range(4)]
    for ph in range(4):
        for pat in range(16):
            idx = 0
            for j in range(4):
                bit = (pat >> (3 - j)) & 1        # j = 0 is the oldest dot
                idx |= bit << ((ph + 1 + j) & 3)  # its phase position
            tbl[ph][pat] = idx
    return tbl


_AP2_IDX = _ap2_idx_table()


def _ap2_line_bits(native, y):
    """One scanline's 560 dots from the interleaved pages: byte column k
    is aux for even k, main for odd, seven dots per byte, bit i the
    (7k+i)th dot; bit 7 plays no part in DHGR."""
    bits = []
    for k in range(80):
        byte = (native["aux"] if k % 2 == 0 else native["main"])[y][k >> 1]
        for i in range(7):
...
class _Ap2:
    ...
    def pack(native):
        ...
        for y in range(native["h"]):
            aux += bytes(native["aux"][y])
            main += bytes(native["main"][y])
        return [(SEC_BITMAP, 0, bytes(aux)), (SEC_SCREEN, 0, bytes(main))]

    @staticmethod
    def unpack(sections, w, h):
        s = _sections_by_type(sections)
        a, m = s[SEC_BITMAP], s[SEC_SCREEN]
        return {"w": w, "h": h,
                "aux": [list(a[y * 40:y * 40 + 40]) for y in range(h)],
                "main": [list(m[y * 40:y * 40 + 40]) for y in range(h)]}

    @staticmethod
    def render(native, w, h):
        # The preview IS the signal model: the same window walk the wire
        # performs, so sequences between the sixteen render as the
        # emulator shows them. 560 dots wide (two per master column),
        # and each scanline DOUBLED so the preview keeps the master's
        # proportions: the signal class is judged by eye on exactly
        # these files, and a distorted aspect hides the verdict.
        rows = []
        for y in range(h):
            bits = _ap2_line_bits(native, y)
            row = []
            win = 0
            for x in range(560):
                win = ((win << 1) | bits[x]) & 0xF
                row.append(_AP2_PAL[_AP2_IDX[x & 3][win]])
            rows.append(row)
    ...
    @staticmethod
    def pattern(w, h):
        aux = [[(3 * y + x) % 128 for x in range(40)] for y in range(h)]
        main = [[(5 * y + 2 * x + 1) % 128 for x in range(40)]
                for y in range(h)]
        return {"w": w, "h": h, "aux": aux, "main": main}
...
def _collect_numbered(sources, broad=False):
    ...
    directory listed before it. With broad, a directory's masters-broad/
    subdirectory overrides same-numbered pictures (the two master classes;
    an explicit file source is always taken verbatim)."""
    entries = {}
    for src in sources:
        if os.path.isdir(src):
            for name in sorted(os.listdir(src)):
                ...
                if iid is not None:
                    ...
                    if broad:
                        alt = os.path.join(src, "masters-broad", name)
...
def _convert_job(job):
    ...
    iid, path, tag, dest, preview, codec, c64src = job[:7]
    zx_route = job[7] if len(job) > 7 else None
    try:
        if zx_route:
            # The route rides the job so it survives a spawn-started
            # worker (a parent-set global would not).
            global _ZX3_ROUTE
            _ZX3_ROUTE = zx_route
...
def cmd_convert(args) -> int:
    ...
    zx_route = "direct" if getattr(args, "zx_colour", False) else None
    broad_used = sum(1 for p in entries.values()
                     if os.path.basename(os.path.dirname(p)) == "masters-broad")
    if broad_used:
        print(f"arcimg: {broad_used} of {len(entries)} masters from "
              f"masters-broad/ (the broad-pixel set)")
    jobs = []
    skipped = 0
    for iid in sorted(entries):
...
def cmd_scr(args) -> int:
    ...
    if src.lower().endswith(".png"):
        try:
            if getattr(args, "zx_colour", False):
                global _ZX3_ROUTE
                _ZX3_ROUTE = "direct"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_conv.add_argument("--zx-colour", action="store_true",
                        help="ZX3 only: the colour conversion path (the "
                             "direct route: pixels 1:1, hue-first snap, "
                             "ink over black paper) instead of the mono "
                             "Canopus form, for art that is committed "
                             "palette regions already")
    ...
    p_scr.add_argument("--zx-colour", action="store_true",
                       help="convert a master PNG through the colour path "
                            "(the Rabenstein route), as convert --zx-colour")
