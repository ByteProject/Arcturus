            ...
            for yy in range(8):
                for xx in range(4):
                    ...
                    if c in allowed:
                        ...
                        code = _nearest(_COLODORE[c], argb)
...
def _convert_cpc(rows, salient=None):
    # Mode 0: 160 wide, 2:1 pixels, 16 inks freely chosen from the
    # 27-color cube, no cells. Polizei's recipe (m_cpcm0.pde) with R3's
    # ruled metric restored: pixels map to the cube with the CHROMA-
    # DUMPING distance (a dusk purple keeps its family: the cube has no
    # dark purple, and by plain distance (100,76,138) lands on the grey
    # (128,128,128), the standing grey-sky complaint, finally traced; at
    # weight 3 the grey still won by four percent, so the weight is 4),
    # the sixteen most frequent cube colors become the inks, stragglers
    # remap to their nearest kept ink.
    lv = (0, 0x80, 0xFF)
    cube = [(lv[r], lv[g], lv[b])
    ...
    idx = _collapse_pairs([[_nearest(c, cube, metric=dist_cpc)
        ...
        for i in row:
            if i in order:
                ...
                if i not in stray:
                    stray[i] = order[min(
                        inks,
                        key=lambda k: dist_cpc(cube[i], cube[k]))]
                out.append(stray[i])
        pixels.append(out)
    if salient:
        bright = order[max(inks, key=lambda k: 2 * cube[k][0]
                           + 4 * cube[k][1] + cube[k][2])]
        _force_disc(pixels, salient, bright)
    pal = list(inks)
    while len(pal) < 16:
        pal.append(0)  # unused slots; a duplicate ink is legal hardware
    return {"w": len(pixels[0]), "h": len(pixels), "pixels": pixels,
            "palette": pal, "regs": [0]}
