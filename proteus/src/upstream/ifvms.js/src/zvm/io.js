...
module.exports = {
	...
	// arc_image (Proteus): EXT:0x80 draw_image image-id mode. The band
	// itself is the shell's business; the engine only forwards the call.
	// With no handler installed this is a silent no-op, which is exactly
	// what the Standard asks of unknown private EXT opcodes.
	draw_image: function( id, mode )
	{
		if ( this.options.arc_image_draw )
		{
			this.options.arc_image_draw( id, mode );
		}
	},
	...
	update_header: function()
	{
		...
		ram.setUint8( 0x01,
			...
			// arc_image (Proteus): bit 1 advertises pictures when the shell
			// installed a draw handler; re-stamped here on restart and
			// restore, as the contract requires. Without a handler the bit
			// is preserved as before (the Inform quirk).
			| (this.options.arc_image_draw ? 0x02 : (ram.getUint8(0x01) & 0x02))
