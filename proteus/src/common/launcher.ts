...
import {setup_arc_image} from './arcimage.js'
        ...
        try {
            ...
            this.loading_pane = new LoadingPane({
                ...
                props: {
                    ...
                    // Proteus: an embedded story's path is a bare filename
                    // with no separator, which upstream's regex never met;
                    // fall back to the path itself.
                    title: story.title ?? (/([/=])([^/=]+)$/.exec((story.path || story.url)!)?.[2] ?? (story.path || story.url)!),
                },
            })
    ...
    load = async () => {
        try {
            ...
            const requires = await Promise.all([
                ...
                // Proteus: the emglken file-size table went with the wasm
                // engines; zvm.js is small enough to need no progress entry.
                ...engine.load.map(path => fetch_resource(this.options, path, progress_callback)),
            ])
            ...
            // Proteus: when the Blorb declares arc_image pictures (ARCI),
            // build the band and hand the engine the draw handler; its
            // presence is also what lights the capability bit.
            options.arc_image_draw = setup_arc_image(options.Blorb)

            // Proteus: ZVM needs a GlkApi instance; AsyncGlk is the one
            // that speaks to this shell's GlkOte. Its file side wants the
            // classic synchronous Dialog (localStorage saves), while the
            // launcher keeps its async Dialog for story loading, so both
            // ride the options under different names.
            options.Glk = new AsyncGlk()
            WebDialog.init({GlkOte: options.GlkOte} as any)
            options.classic_dialog = WebDialog
