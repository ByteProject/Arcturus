...
export async function process_index_html(options: SingleFileOptions, files: Map<string, Uint8Array<ArrayBuffer>>): Promise<string> {
    ...
    if (story) {
        ...
        if (story.title) {
            ...
            if (story.author) {
                inclusions.push(
                    `<meta property="og:site_name" content="Proteus"/>`,
    ...
    // Proteus: the page title is the interpreter and the story's
    // filename, the Actaea manner (Stefan's ruling). Without a story it
    // stays "Proteus".
    if (story?.filename) {
        indexhtml = indexhtml.replace(/<title.+?\/title>/, `<title>Proteus - ${escape(story.filename)}</title>`)
    }
